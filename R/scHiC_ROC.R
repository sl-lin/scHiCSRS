...
scHiC_ROC <- function(observed, expected, result){
  ...
  auc=format(round(AUC(c(1,1-output$PTSZ,0),c(1,output$PTDO,0)),2), nsmall = 2)

  #return(output)
  plot(c(1,1-output$PTSZ,0),c(1,output$PTDO,0), type = "l", xlab = "1-PTSZ", ylab = "PTDO",
