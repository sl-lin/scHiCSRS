...
#'
#' @param result Output of SRS for simulated data or the organized results of real data.
...
scHiC_assess <- function(result, cell_index=1, n, cell_type, dims = 2,perplexity=10, seed=1000,
  ...
  if(is.null(expected)){##### for real data analysis
    ...
    p4=scHiC_tSNE(imputed, cell_type=cell_type,
                  dims = dims, perplexity=perplexity, seed=seed, title="scHiCSRS",
                  kmeans = kmeans, ncenters = ncenters)
    ...
    for(j in 1:ncol(observed)){
      ...
      MAEZ[[j]]=abs(predictv-Truevalue)
      MAEA[[j]]=abs(result$Impute_SZ[,j]-expected[,j])
      MCA[[j]]=cor(result$Impute_SZ[,j],observed[,j])
    ...
    summa_mean=data.frame(PTSZ=mean(PTSZ),PTDO=mean(PTDO),
                          MAEZ=mean(MAEZ),MAEA=mean(MAEA),
                          MCA=mean(MCA))
    summa_se=data.frame(PTSZ=sd(PTSZ)/sqrt(length(PTSZ)),PTDO=sd(PTDO)/sqrt(length(PTDO)),
                        MAEZ=sd(MAEZ)/sqrt(length(MAEZ)),MAEA=sd(MAEA)/sqrt(length(MAEA)),
                        MCA=sd(MCA)/sqrt(length(MCA)))
    ...
    fig_label("B", cex=2)
    scHiC_hm(result$Impute_SZ[,cell_index], n, title="scHiCSRS")
    fig_label("C", cex=2)
    ############# ROC curve
